...
args=(--file "$ROOT/docker/cflinuxfs5-builder/Dockerfile" --target output --tag "cf-herdr-cflinuxfs5-builder:${BUILD_TAG:-local}" --output "type=local,dest=$staging" --build-arg "TARGETARCH=$TARGETARCH" --build-arg "GOARCH=$GOARCH" --build-arg "GOOS=${GOOS:-linux}" "${artifact_args[@]}")
