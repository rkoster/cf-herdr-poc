...
func TestEnvironmentOverridesAmbientRuntimePaths(t *testing.T) {
	base := []string{
		...
		"COLLIE_STATE_DIR=/ambient/collie", "HERDR_SOCKET_PATH=/ambient/socket", "COLLIE_HOST=0.0.0.0", "COLLIE_PORT=9999", "COLLIE_PACK_TRANSPORT=pinned",
	}
	env := Environment(Runtime{ConfigDir: "/manager/config", StateDir: "/manager/state", SocketPath: "/manager/herdr.sock", Host: "127.0.0.2", Port: 8787, PackTransport: "cf-identity"}, base)
	want := map[string]string{
		"PATH": "/bin", "HERDR_PLUGIN_CONFIG_DIR": "/manager/config", "HERDR_PLUGIN_STATE_DIR": "/manager/state",
		"COLLIE_STATE_DIR": "/manager/state", "HERDR_SOCKET_PATH": "/manager/herdr.sock", "COLLIE_HOST": "127.0.0.2", "COLLIE_PORT": "8787", "COLLIE_PACK_TRANSPORT": "cf-identity",
	}
